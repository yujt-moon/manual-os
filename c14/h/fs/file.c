...
int32_t file_create(struct dir* parent_dir, char* filename, uint8_t flag) {
     ...
     * 因为 file_table 数组中的文件描述符的 inode 的指针要指向它。 */
...
int32_t file_open(uint32_t inode_no, uint8_t flag) {
    ...
    if (flag & O_WRONLY || flag & O_RDWR) { // 只要是关于写文件，判断是否有其他进程正写此文件；若是读文件，不考虑 write_deny
        /* 以下进入临界区前先关中断 */
...
int32_t file_write(struct file* file, const void* buf, uint32_t count) {
        ...
        if (file_will_use_blocks <= 12) {
            ...
            while (block_idx < file_will_use_blocks) {
                ...
                if (block_idx < 12) {       // 新创建的 0 ~ 11 块直接存入 all_blocks 数组
                    ASSERT(file->fd_inode->i_sectors[block_idx] == 0);      // 确保尚未分配扇区地址
                    file->fd_inode->i_sectors[block_idx] = all_blocks[block_idx] = block_lba;
                } else {        // 间接块只写入到 all_blocks 数组中，待全部分配完成后一次性同步到硬盘
            ...
            indirect_block_table = file->fd_inode->i_sectors[12];      // 获得一级间接表地址

            /* 已经使用的间接块也将被读入 all_blocks，无须单独收录 */
    ...
    /* 块地址已经收集到 all_blocks 中，下面没开始写数据 */
...
/* 从文件 file 中读取 count 个字节写入 buf，返回读出的字节数，若文件尾则返回-1 */
int32_t file_read(struct file* file, void* buf, uint32_t count) {
    ...
    int32_t indirect_block_table;       // 用来获取一级间接表地址
    uint32_t block_idx;                 // 获取待读块地址

    /* 以下开始构建 all_blocks 块地址数组，专门存储用到的块地址（本程序中块大小同扇区大小） */
    if (read_blocks == 0) {         // 在同一个扇区内读数据，不涉及跨扇区读取
        ...
        if (block_read_end_idx < 12) {      // 数据结束所在的块属于直接块
            ...
            ide_read(cur_part->my_disk, indirect_block_table, all_blocks + 12, 1);      // 将一级间接块表读进来写入到第十三块位置之后
        }
    }
