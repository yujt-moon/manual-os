...
struct dir_entry* dir_read(struct dir* dir) {
    ...
    uint32_t dir_entry_size = cur_part->sb->dir_entry_size;
    uint32_t dir_entries_per_sec = SECTOR_SIZE / dir_entry_size;    // 1扇区内可容纳的目录项个数
    ...
    while (block_idx < block_cnt) {
        ...
        /* 遍历扇区内所有目录项 */
        while (dir_entry_idx < dir_entries_per_sec) {
            if ((dir_e + dir_entry_idx)->f_type) {      // 如果 f_type 不等于 0,即不等于 FT_UNKNOWN
...
int32_t dir_remove(struct dir* parent_dir, struct dir* child_dir) {
    ...
    /* 回收 inode 中 i_sectors 所占用的扇区，并同步 inode_bitmap 和 block_bitmap */
