...
static bool mount_partition(struct list_elem* pelem, int arg) {
    ...
    if (!strcmp(part->name, part_name)) {
         ...
         * 只有返回 true 时，list_traversal 才会停止遍历，减少后面元素无意义的遍历。 */
        return true;
    }
...
static void partition_format(struct partition* part) {
    ...
    uint32_t inode_bitmap_sects = DIV_ROUND_UP(MAX_FILES_PER_PART, BITS_PER_SECTOR);    // i结点位图占用的扇区数，最多支持 4096 个文件
    ...
    printk("    magic:0x%x\n    part_lba_base:0x%x\n    all_sectors:0x%x\n    inode_cnt:0x%x\n    "
           ...
           sb.block_bitmap_sects, sb.inode_table_lba, sb.inode_table_sects, sb.data_start_lba);
    ...
    /* 1 先将位图最后一字节到其所在的扇区的结束全置为 1，即超出实际块数的部分直接为已占用 */
     ...
     * 即 inode_bitmap_sects 等于 1,所以位图中的位全部代表 inode_table 中的 inode，
    ...
    i->i_sectors[0] = sb.data_start_lba;    // 由于上面的 memset, i_sectors 数组的其他元素都初始化为 0
...
int32_t sys_open(const char* pathname, uint8_t flags) {
    ...
    if (pathname_depth != path_searched_depth) {        // 说明并没有访问到全部的路径，某个中间目录是不存在的
        printk("pathname_depth = %x, path_searched_depth = %x", pathname_depth, path_searched_depth);
    ...
    /* 此 fd 是指任务 pcb->fd_table 数组中元素下标，
...
/* 将 buf 中连续 count 个字节写入文件描述符，成功则返回写入的字节数，失败返回 -1 */
...
int32_t sys_lseek(int32_t fd, int32_t offset, uint8_t whence) {
    ...
    switch (whence) {
        ...
        /* SEEK_CUR 新的读写位置是相对于当前位置增加 offset 个位移量 */
...
int32_t sys_mkdir(const char* pathname) {
    ...
    if (inode_no != -1) {       // 如果找到了同名目录或文件，失败返回
        ...
        uint32_t path_searched_depth = path_depth_cnt(searched_record.searched_path);
        /* 先判断是否把 pathname 的各层目录都访问到了，即是否在某个中间目录就失败了 */
        if (pathname_depth != path_searched_depth) {    // 说明并没有访问到全部的路径，说明某个中间目录是不存在的
    ...
    struct dir* parent_dir = searched_record.parent_dir;
    /* 目录名称后可能会有字符 '/'，所以最好直接使用 searched_record.searched_path，无'/' */
    ...
    memset(io_buf, 0, SECTOR_SIZE * 2);     // 清空 io_buf
    if (!sync_dir_entry(parent_dir, &new_dir_entry, io_buf)) {      // sync_dir_entry 中将 block_bitmap 通过 bitmap_sync 同步到硬盘
    ...
    /* 创建文件或目录需要创建相关的多个资源，若某步失败会执行到下面的回滚步骤 */
    rollback:       // 因为某步骤操作失败而回滚
    switch (rollback_step) {
        case 2:
            bitmap_set(&cur_part->inode_bitmap, inode_no, 0);   // 如果新文件的 inode 创建失败，之前位图中分配的 inode_no 也要恢复
        case 1:
            /* 关闭所创建目录的父目录 */
            dir_close(searched_record.parent_dir);
            break;
...
/* 读取目录 dir 的1个目录项，成功后返回其目录项地址，到目录尾或出错时返回 NULL */
    ...
    if (inode_no == -1) {
        ...
        if (searched_record.file_type == FT_REGULAR) {
            ...
            if (!dir_is_empty(dir)) {       // 非空目录不可删除
                printk("dir %s is not empty, it is not allowed to delete nonempty directory!\n", pathname);
            } else {
...
/* 在 inode 编号为 p_inode_nr 的目录中查找 inode 编号为 c_inode_nr 的子目录的名字，将名字存入缓冲区 path，成功返回 0,失败返回 -1 */
...
char* sys_getcwd(char* buf, uint32_t size) {
    ...
    while ((last_slash = strrchr(full_path_reverse, '/'))) {
        ...
        /* 在 full_path_reverse 中添加结束字符，作为下一次执行 strcpy 中的 last_slash 的边界 */
        *last_slash = 0;
    }
    sys_free(io_buf);
    return buf;
    ...
    while (channel_no < channel_cnt) {
        ...
        while (dev_no < 2) {
            ...
            while (part_idx < 12) {     // 4 个主分区+ 8 个逻辑分区
                ...
                /* channels 数组是全局变量，默认值为 0，disk 属于嵌套结构，
                 * partition 又为 disk 的嵌套结构，因此 partition 中的成员默认也为 0。
                 * 若 partition 未初始化，则 partition 中的成员仍为 0。下面处理存在的分区 */
