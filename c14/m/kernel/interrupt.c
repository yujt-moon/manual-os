...
struct gate_desc {
    ...
    uint16_t    selector;
    uint8_t     dcount;         // 此项为双字技术字段，是门描述符中的第 4 个字节，此项固定值，不用考虑
    uint8_t     attribute;
...
static struct gate_desc idt[IDT_DESC_CNT];      // idt 是中断描述符表，本质上就是中断门描述符数组
...
static void general_intr_handler(uint8_t vec_nr) {
    if (vec_nr == 0x27 || vec_nr == 0x2f) { // IRQ7 和 IRQ15 会产生伪中断（spurious interrupt），无需处理 0x2f 是从片 8259A 上的最后一个 IRQ 引脚，保留项
        return;
    }
...
enum intr_status intr_disable() {
    ...
    if (INTR_ON == intr_get_status()) {
        old_status = INTR_ON;
        asm volatile ("cli" : : : "memory");    // 关中断，cli指令将 IF 位置 0
        return old_status;
    } else {
        old_status = INTR_OFF;
        return old_status;
