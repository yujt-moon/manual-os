 ...
 * 一个页框大小的位图可以表示 128M 内存，位图位置安排在地址 0xc009a000，
 * 这样本系统最大支持 4 个页框的位图，即 512M */
...
#define PTE_IDX(addr) ((addr & 0x003ff000) >> 12)
 ...
 /* 0xc0000000 是内核从虚拟地址 3G 起。0x100000 意指跨过低端 1MB 内存，是虚拟地址在逻辑上连续 */
...
struct arena {
    ...
    /* large 为 true 时，cnt 表示的是页框数。否则 cnt 表示空闲 mem_block 数量 */
    uint32_t cnt;
    bool large;
};
...
static void* vaddr_get(enum pool_flags pf, uint32_t pg_cnt) {
    ...
    if (pf == PF_KERNEL) {
        ...
        /* (0xc0000000 - PG_SIZE) 作为用户 3 级栈已经在 start_prrocess 被分配 */
...
uint32_t* pte_ptr(uint32_t vaddr) {
    /* 先访问到页表自己 + \
     * 再用页目录 pde （页目录内页表的索引）作为 pte 的索引访问到页表 + \
     * 再用 pte 的索引作为页内偏移 */
 ...
 *************************************************************/
    ...
    if (*pde & 0x00000001) {    // 页目录项和页表项的第 0 位为 P，此处判断目录项是否存在
        ...
        } else {    // 目前应该不会执行到这，因为上面的 ASSERT 会先执行，关闭调试下面的 PANIC 会起作用
            PANIC("pte repeat");
        }
 ...
 *******************************************************************/
    ...
    /* 因为虚拟地址是连续的，但是物理地址可以不连续，所以逐个坐映射 */
    while (cnt-- > 0) {
        void* page_phyaddr = palloc(mem_pool);
        if (page_phyaddr == NULL) {         // 失败时要将曾经已经申请的虚拟地址和物理页全部回滚，在将来完成内存回收时再补充
            return NULL;
        }
...
static void mem_pool_init(uint32_t all_mem) {
    ...
    uint32_t page_table_size = PG_SIZE * 256;       // 页表大小 = 1 页的页目录表 + 第 0 和第 768 个页目录指向同一个页表 +
...
void* sys_malloc(uint32_t size) {
    ...
    if (size > 1024) {
        ...
        if (list_empty(&descs[desc_idx].free_list)) {
            ...
            /* 对于分配的小块内存，将 desc 置为相应内存块描述符，cnt 置为此 arena 可用的内存块数，large 置为 false */
        ...
        a = block2arena(b);     // 获取内存 b 所在的 arena
        a->cnt--;               // 将此 arena 中的空闲内存块减 1
        lock_release(&mem_pool->lock);
        return (void*)b;
    }
}
...
void mfree_page(enum pool_flags pf, void* _vaddr, uint32_t pg_cnt) {
    ...
    /* 确保待释放的物理内存在低端 1M+1k 大小的页目录地址范围外 */
